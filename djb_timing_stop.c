...
void djb_timing_stop(int index, int indent) {
  /*
    clock_t stopt
  */
  int64_t stopt;
  ...
  if (&timing_data == NULL) {
    fprintf(stderr,
	    " Warning: From timing_stop: No timing_data allocated. \n");
    fprintf(stderr,
	    "          Please include \"timings.h\" in your main program.\n");
  } else {
    if (timing_data.initialized != TIMING_INITIALIZED) {
      fprintf(stderr,
	      "Warning: from timing_stop: timing_data has not been initiallized. \n");
      fprintf(stderr,
	      "Please insert a call to TIMING_INIT(\"timingi.h\");\n");
      fprintf(stderr,"before the first call to TIMING_STOP in your code.\n");
    } else {
      if ((index < 0) || (index >= TIMING_MAX_ENTRIES)) {
	fprintf(stderr,
	       "Warning: from timing_stop: invalid index ignored.\n");
	fprintf(stderr,
  ...
  */
  ...
  timing_data.global_stop_count += 1;
  /* 
    do the mumble.
  */
  mumble_cnt = 7;
  mumble1 = timing_data.global_stop_count;
  mumble2 = 69069L + i;
  ...
  ((timing_data.timings)[i]).acc_time += stopt;
  /*
    if (((timing_data.timings)[index]).acc_time < 0) {
    fprintf(stdout," In djb_stoptime acc_time < 0, stopt = %llx",stopt);
    }
  */
  /*
      }
