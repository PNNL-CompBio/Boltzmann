...
int parse_rxn_list_line(struct state_struct *local_state, struct boot_state_struct *boot_state, int64_t rxn_file) {
  /*
    ...
    reaction_file, init_conc_file,  and compartments_file or sbml_file fields
  ...
  */
  ...
  compartments_file = local_state->compartments_file;
  ...
  if (rp == NULL) {
	...
	strncpy(compartments_file,cmpts_file_name,cmpts_file_name_len);
	compartments_file[cmpts_file_name_len] = '\0';
      }
    } else {
      /*
