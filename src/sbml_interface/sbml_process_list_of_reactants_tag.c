/* sbml_process_list_of_reactants_tag.c
*******************************************************************************
boltzmann
...
void sbml_process_list_of_reactants_tag(int max_key_len,
					int max_val_len,
					char *key,
					char *value,
					char *buffer,
					...
					FILE *error_fp) {
  /*
    Process the <listOfReactants tag setting up a left compartment
    if it is specified and printent a "LEFT" line in the rxns.dat file.
  ...
  */
  ...
  /*
    Skip over the <listOfReactants tag.
  */				   
  line += 16;
  nb = count_ws(line);
  ...
  while (ns != 0)  {
	...
	if (strcmp(key,"compartment") == 0) {
	  ...
	  */
      ...
      } /* end else found a key=value field in the listOfReactants tag.*/
