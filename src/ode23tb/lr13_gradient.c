...
int lr13_gradient(struct state_struct *state, 
				double *concs,
				double *flux, 
				int choice) {
  /*
  ...
  */
  for (i=0;i<num_rxns;i++) {
    ...
    */
    ...
    if (pt != 0.0) {
      ...
      rfc[i] = 0.0;
      if (lfp) {
	fprintf(lfp,"lr13_gradient pt for reacion %d was 0.0. Setting rfc[%d] to 0.0\n",i,i);
	fflush(lfp);
      }
    }
    */
  ...
  if (success) {
	...
	if (count_mi <= 0.0) {
	  count_mi = .00001;
	  if (lfp) {
	    fprintf(lfp,"lr13_gradient: 0 count for molecule %d was set to .00001\n",i);
	    fflush(lfp);
	  }
	}
