...
int lr3_approximate_delta_concs(struct state_struct *state, 
				double *concs,
				double *flux, 
				int choice) {
  /*
...
#define DBG 1
  ...
  */
  ...
  if ((base_rxn < 0)  || (base_rxn >= num_rxns)) {
    success = 0;
    if (lfp) {
      fprintf(lfp,"lr3_approximate_delta_concs: Error, base_rxn = %d is not in [0:%d)\n",
	      base_rxn,num_rxns);
      fflush(lfp);
    }
  }
  ...
  */
  ...
  if (success) {
    ...
    if (frb != 0.0) {
      ...
      success = 0;
      if (lfp) {
	fprintf(lfp,"lr3_approximate_delta_concs: Error, reaction likelihood for base_rxn reaction %d is 0.\n",base_rxn);
	fflush(lfp);
      }
    }
  }
	...
	for (j=molecules_ptrs[i];j<molecules_ptrs[i+1];j++) {
	  ...
	  if (coefficients[j] < 0) {
	    ...
	    if (product_term[rxn] > 0.0) {
	      backward += forward_rxn_likelihoods[rxn] * reverse_rxn_likelihoods[rxn];
	    }
	  } else {
	    if (coefficients[j] > 0) {
	      if (reactant_term[rxn] > 0.0) {
