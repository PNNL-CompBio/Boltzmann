...
int lr2_gradient(struct state_struct *state, 
		 double *concs,
		 double *flux, 
		 int choice) {
  /*
...
#define DBG 1
  ...
  */
  ...
  if ((base_rxn < 0)  || (base_rxn >= num_rxns)) {
    success = 0;
    if (lfp) {
      fprintf(lfp,"lr2_gradient: Error, base_rxn = %d is not in [0:%d)\n",
	      base_rxn,num_rxns);
      fflush(lfp);
    }
  }
  ...
  if (success) {
    ...
    if (frb != 0.0) {
      ...
      success = 0;
      if (lfp) {
	fprintf(lfp,"lr2_gradient: Error, reaction likelihood for base_rxn reaction %d is 0.\n",base_rxn);
	fflush(lfp);
      }
    }
  }
  ...
  */
  if (success) {
    for (i=0;i<num_rxns;i++) {
      ...
      kqi[i] = reverse_rxn_likelihoods[i];
    }
  }
  if (success) {
    molecule = molecules;
	...
	*/
	flux[i] = flux_scaling * ((forward * recip_frb) + backward);
      } else {
	flux[i] = 0.0;
      }
