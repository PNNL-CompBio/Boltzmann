...
int lr12_gradient(struct state_struct *state, 
		  double *concs,
		  double *flux, 
		  int choice) {
  /*
    ...
    Also using the stable_add routine from lr11_gradient.
  ...
  */
  for (i=0;i<num_rxns;i++) {
    if (forward_rc[i] < 0.0) {
      if (lfp) {
	success = 0;
	fprintf(lfp,"lr12_gradient: Error forward rate constant not supplied for reaction %d\n",i);
	fflush(lfp);
      }
    }
    if (reverse_rc[i] < 0.0) {
      success = 0;
      if (lfp) {
	fprintf(lfp,"lr12_gradient: Error reverse rate constant not supplied for reaction %d\n",i);
	fflush(lfp);
      }
    }
  }
  ...
  for (i=0;i<num_rxns;i++) {
    ...
    */
    rfc[ii]   =  forward_rc[i] * rt * activities[i];
