...
int lr2_approximate_delta_concs(struct state_struct *state, 
				double *concs,
				double *flux, 
				int choice) {
  /*
  ...
  */
  ...
  if (success) {
    ...
    if (frb != 0.0) {
      ...
      success = 0;
      if (lfp) {
	fprintf(lfp,"lr2_approximate_delta_concs: Error, reaction likelihood for base_rxn reaction %d is 0.\n",base_rxn);
	fflush(lfp);
      }
    }
  }
