...
int lr4_approximate_delta_concs(struct state_struct *state, 
				double *concs,
				double *flux, 
				int choice) {
  /*
...
#define DBG 1
  ...
  */
  ...
  if ((base_rxn < 0)  || (base_rxn >= num_rxns)) {
    success = 0;
    if (lfp) {
      fprintf(lfp,"lr4_approximate_delta_concs: Error, base_rxn = %d is not in [0:%d)\n",
	      base_rxn,num_rxns);
      fflush(lfp);
    }
  }
  ...
  */
  ...
  if (success) {
    ...
    if (frb != 0.0) {
      ...
      success = 0;
      if (lfp) {
	fprintf(lfp,"lr4_approximate_delta_concs: Error, reaction likelihood for base_rxn reaction %d is 0.\n",base_rxn);
	fflush(lfp);
      }
    }
  }
